...
result_path = path+'results_summer/'
if osp.exists(result_path) == False:
    os.makedirs(result_path)   
    
dataDir = 'satellite_imagery/'
ICESatDr = 'ICESat/summer/tracks/'
...
metrics = pd.DataFrame(data = {'regressor':Model,'R':Rgroup,
                                ...
                                'count':x.shape[0]})
metrics.to_csv(result_path+'regressionMetrics_original_'+Model+'_'+sampleStr+'_'+beam+'_summer.csv')
...
plt.tight_layout()
plt.show()
plt.savefig(result_path+'CHMregression_'+sampleStr+'_original_LidarArea_'+Model+'_'+beam+'_summer.png',dpi = 500)
