...
#% determine the threshold beyond 1.5 IQR
# from scipy.stats import f
# p = len(model.params) -1
# n = x.shape[0]
# thres = f.ppf(0.5, p, n-p)
# print(thres)
# thres1 = (np.nanmean(cooks[0])*2)
...
# plt.tight_layout()
# plt.show()
# plt.savefig(result_path+'CookDist_'+beam+'_intersectingPts_new.png',dpi = 500)
...
CHMd_nonNaN_idx = pd.read_csv(result_path+'ALS_ICESat_intersectingPts.csv')
...
t = ax.text(0.525, 0.25, textstr, transform=ax.transAxes, fontsize=12,verticalalignment='top', bbox=props)
...
plt.tight_layout()
plt.show()
plt.savefig(result_path+'CHMregression_'+sampleStr+'_ours_excludingIntersectPts_'+Model+'.png',dpi = 500)
