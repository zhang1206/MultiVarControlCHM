...
flag_CHU = RHcanopy['canopy_h_u']<np.nanmean(RHcanopy['canopy_h_u'])
Filter= flag_CHU
RHcanopy_validICESat = RHcanopy[Filter]
print(RHcanopy_validICESat.shape[0])
...
metrics = pd.DataFrame(data = {'regressor':Model,'R':Rgroup,
                                ...
                                'count':x.shape[0]})
metrics.to_csv(result_path+'regressionMetrics_compared_'+Model+'_'+sampleStr+'_'+beam+'.csv')
...
plt.tight_layout()
plt.show()
plt.savefig(result_path+'CHMregression_'+sampleStr+'_compared_LidarArea_'+Model+'_'+beam+'.png',dpi = 500)
