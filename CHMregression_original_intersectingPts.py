...
CHMrefer = io.imread(path + dataDir +'CHM_RH.tif')[:,:,3] # RH95
...
S1_VH_median = io.imread(path + dataDir + 'S1_VH_median.tif')
S1_VV_median = io.imread(path + dataDir + 'S1_VV_median.tif')
...
CHMrefer = io.imread(path + dataDir +'CHM_RH.tif')[:,:,3]
Flag_lc = (NLCD < 40)
...
metrics = pd.DataFrame(data = {'regressor':Model,'R':Rgroup,
                                ...
                                'count':x.shape[0]})
metrics.to_csv(result_path+'regressionMetrics_original_'+Model+'_'+sampleStr+'.csv')
...
plt.tight_layout()
plt.show()
plt.savefig(result_path+'CHMregression_'+sampleStr+'_original_LidarArea_'+Model+'.png',dpi = 500)
