...
result_path = path+'results_studyRegion3_summer/'
if osp.exists(result_path) == False:
    os.makedirs(result_path)   
    
dataDir = 'Data_2019_2020/'
ICESatDr = 'ICESat_summer/'
...
#%% -----------------compared study----------------------------------
ICESatPts = gpd.read_file(path+dataDir+ICESatDr+'ICESat_all_proj2.shp')
...
flag_validCH = (RHcanopy['RH100']<40)&(RHcanopy['RH100']>3)
...
CHMrefer = io.imread(path + dataDir +'previousData/ALS_CHM_30m_RH_merged.tif')[:,:,3]
Flag_lc = (NLCD < 40)
Flag_validALS = CHMrefer<3
CHMrefer = np.where(Flag_lc|Flag_validALS, np.nan,CHMrefer)


CHMd_nonNaN_idx = pd.read_csv(path+'results_studyRegion3_allseason/'+'ALS_ICESat_intersectingPts.csv')
ICESat_RowIdx = CHMd_nonNaN_idx[['RowIdx']].to_numpy()
ICESat_ColumnIdx = CHMd_nonNaN_idx[['ColumnIdx']].to_numpy()
ICESat_location = np.zeros((CHMrefer.shape[0],CHMrefer.shape[1]))
ICESat_location[ICESat_RowIdx,ICESat_ColumnIdx] = 1

Unknown_RowIdx, Unknown_ColumnIdx = np.where(~np.isnan(CHMrefer) & (ICESat_location == 0))
...
metrics = pd.DataFrame(data = {'regressor':Model,'R':Rgroup,
                                ...
                                'count':x.shape[0]})
metrics.to_csv(result_path+'regressionMetrics_original_'+Model+'_'+sampleStr+'_summertracks.csv')
...
ax.set_ylim([3,35])
ax.set_xlim([3,35])
cbar = fig.colorbar(h[3], ax=ax,ticks=([50,100,150,200,250,300]))
tick_font_size = 12
cbar.ax.tick_params(labelsize=tick_font_size)

ax.plot(Unknown_pred_rf,ypred,'-r',linewidth=1.5)
ax.plot([3,35],[3,35],'--k', linewidth=1.5)
...
t = ax.text(0.52, 0.25, textstr, transform=ax.transAxes, fontsize=12,verticalalignment='top', bbox=props)
...
plt.tight_layout()
plt.show()
plt.savefig(result_path+'CHMregression_'+sampleStr+'_original_excludingIntersectPts_'+Model+'_summertracks.png',dpi = 500)
